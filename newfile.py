...
with sqlite3.connect('student_db.db') as db2:
	d=db2.cursor()
	d.execute('CREATE TABLE IF NOT EXISTS student(name TEXT NOT NULL PRIMARY KEY,branch TEXT,regd_no INTEGER);')
	db2.commit()
	db2.close()
	...
	class main:
		...
		def submit(self):
			with sqlite3.connect('student_db.db') as db:
				c=db.cursor()
				find_student =('SELECT * FROM student WHERE name = ? and branch =? and regd_no =?')
				c.execute(find_student,[(self.name.get()),(self.branch.get()),(self.regd_no.get())])
				if result ==c.fetchall():
			...
			def submit(self):
				with sqlite3.connect('student_db.db') as db:
					db=db.cursor()
					find_student = ('SELECT regd_no FROM student WHERE regd_no =?')
				c.execute(find_student,[(self.regd_no.get())])
				result = c.fetchall()
				if result:
					self.logf.pack_forget()
					self.head['text'] = '\n this is the file'
			...
			def new_student(self):
				...
				if c.fetchall():
					...
					self.log()
				insert = 'INSERT INTO student (name,branch,regd_no) VALUES (?,?,?)'
		...
		def widgets(self):
			...
			Button(self.crf,text='Create Account',bd=3,padx=5,pady=5,command=self.validateAll).grid()
			Button(self.crf,text='Login into existing account',bd =3,padx=5,pady=5,command=self.log).grid(row=2,column=1)
			self.head =Label(self.master,text ='Update Existing Data',pady= 10)
			self.head.pack()
			...
			Label(self.log2f,text ='UPDATE',bd =3,padx =5 ,command=self.submit).grid()
			Entry(self.log2f,text='ADD NEW STUDENT',bd =3,padx=5,pady=5,command=self.cr2).grid(row=1,column=1)
			self.log2f.pack()
			...
			Button(self.cr2f,text='Add new student data',bd=3,padx=5,pady=5,command=self.new_student).grid()
