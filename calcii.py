    ...
    class main:
        ...
        def log6(self):
            ...
            with sqlite3.connect('student_db.db') as db:
                ...
                c.execute(find_user,[(self.regd_no.get())])
                rows = c.fetchall()
            self.log6f=Frame(self.master,padx=10,pady=10)
            tv=ttk.Treeview(self.log6f,columns=(1,2,3,4,5,6,7,8,9),show='headings',height='9')
            tv.grid()
            tv.heading(1,text='Name')
            tv.column('1',minwidth=0,width=100,stretch=NO)
            tv.heading(2,text='Branch')
            ...
            tv.heading(9,text='GRADE')
            tv.column('9',minwidth=0,width=50,stretch=NO)
            Button(self.log6f,text='Go Back',command=self.closelog6).grid()
            for i in rows:
                tv.insert('','end',values=i)
                self.log6f.pack()
        ...
        def widgets(self):
            ...
            Button(self.log4f,text='Show Marksheet',command=self.log6,bd=3,padx=5,pady=25).grid(row=0,column=2)
            Button(self.log4f,text='ADD NEW',command=self.addNew,bd=3,padx=5,pady=25).grid(row=0,column=3)
            Button(self.log4f,text='CLOSE',command=self.closeAll,bd=3,padx=5,pady=25).grid(row=0,column=4)
