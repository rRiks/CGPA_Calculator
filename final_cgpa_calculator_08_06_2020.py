...
with sqlite3.connect('student_db.db') as db:
    ...
    class main:
        ...
        def addSub(self):
            ...
            with sqlite3.connect('student_db.db') as db:
                ...
                if result:
                    ...
                    c.execute(insert , ((self.cet01.get()),(self.cet02.get()),(self.cet03.get()),(self.regd_no.get()),(self.sem_c.get()),(self.name.get()),(self.branch.get()),dict_grade,calculated_cgpa))
                    db.commit()
        ...
        def log3(self):
            ...
            self.name.set(self.n_name.get())
            self.branch.set(self.n_branch.get())
        ...
        def widgets(self):
            ...
            text=Text(self.log3f,height=10,width=40).grid(row=9,columnspan=2)
            Button(self.log3f,text='SUBMIT',command=self.validateMarks,bd=3,padx=5,pady=5).grid(row=6,column=1)
            Button(self.log3f,text='Show Marks',command=lambda: text.insert(END, self.fetchdata())).grid(row=7,column=1)
        ...
        def calc_CG(self):
            ms.showinfo('Current CGPA is ',self.rand())
        ...
        def abcd(self):
            ...
            return dict_grade
        def grade(self):
            ms.showinfo('Your Grade is ',self.abcd())
        ...
        def validateSubmit(self):
            if self.n_name.get() == '':
                ...
                ms.showerror('Information','Please Enter some valid no of swmester completed')
